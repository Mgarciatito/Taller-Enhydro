...
setwd('D:/DIA_3/DIA 3/Datos Grillados/6_Insumos/Pre_practica/')

pp <- read.csv('Pp_practica.csv', header = T, sep = ",")
View(pp)
...
plot(pp$TICLIO,col='blue')
plot(pp$TICLIO, col='blue', ylab= 'Pp(mm)', type='l',
...
write.csv(extraido, 'Pp_extraida.csv')


pp2 <- read.csv('Pp_practica.csv', header = T, sep = ",")
View(pp2)
...
data <- as.xts(read.zoo("Pp_practica.csv", header = TRUE ,sep = ",",
...
hist(coredata(data$TICLIO), freq = T, col='blue')
